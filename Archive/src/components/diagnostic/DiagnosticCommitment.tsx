...
const commitmentLevels = [
  { value: "High", labelKey: "diagnosticCommitment.highLabel", descKey: "diagnosticCommitment.highDesc", Icon: Flame, gradient: "from-orange-500 to-red-600" },
  { value: "Medium", labelKey: "diagnosticCommitment.mediumLabel", descKey: "diagnosticCommitment.mediumDesc", Icon: Lightbulb, gradient: "from-yellow-500 to-amber-600" },
  { value: "Low", labelKey: "diagnosticCommitment.lowLabel", descKey: "diagnosticCommitment.lowDesc", Icon: Search, gradient: "from-blue-500 to-indigo-600" },
];
...
export const DiagnosticCommitment = ({ data, onUpdate, onNext, onBack }: DiagnosticCommitmentProps) => {
  ...
  const handleNext = () => {
    if (!data.commitment_level) {
      toast.error(t('diagnosticCommitment.requiredError'));
      return;
    }
    onNext();
  };
    ...
    <div className="space-y-6 sm:space-y-8 animate-fade-in">
      <div className="text-center space-y-3 sm:space-y-4 px-4">
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-yellow-500/10 to-amber-500/10 border border-yellow-500/20 rounded-full text-sm backdrop-blur-sm">
          <span className="text-yellow-600">{t('diagnosticCommitment.step')}</span>
        </div>
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold bg-gradient-to-r from-yellow-600 via-amber-500 to-yellow-600 bg-clip-text text-transparent">
          {t('diagnosticCommitment.title')}
        </h2>
        <p className="text-base sm:text-lg text-muted-foreground">{t('diagnosticCommitment.subtitle')}</p>
      </div>
            ...
            <div key={level.value} className={`group relative p-4 sm:p-6 rounded-xl border-2 transition-all cursor-pointer animate-fade-in hover:scale-[1.02] ${isSelected ? "border-yellow-500 bg-gradient-to-br from-yellow-500/5 to-amber-500/5 shadow-lg shadow-yellow-500/20" : "border-border hover:border-yellow-500/50 bg-background"}`} style={{ animationDelay: `${index * 100}ms` }} onClick={() => onUpdate({ commitment_level: level.value })}>
              <div className="flex items-start gap-3 sm:gap-4">
                ...
                <div className="flex-1">
                  <h3 className="text-lg sm:text-xl font-bold mb-1 sm:mb-2">{t(level.labelKey)}</h3>
                  <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">{t(level.descKey)}</p>
                </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4 pt-6 sm:pt-8 px-4">
        ...
        <Button onClick={handleNext} disabled={!data.commitment_level} className="w-full sm:w-auto bg-gradient-to-r from-yellow-600 to-amber-600 hover:from-yellow-500 hover:to-amber-500 text-white disabled:opacity-50 disabled:cursor-not-allowed order-1 sm:order-2">{t('common.next')}</Button>
      </div>
    </div>
  );
};
