import { Button } from "@/components/ui/button";
import { Check, Star, Crown, Zap } from "lucide-react";
import { useTranslation } from 'react-i18next';

const Programs = () => {
  const { t } = useTranslation();

  const programs = [
    {
      tier: t('programs.tier1'),
      name: t('programs.name1'),
      price: "49",
      currency: "BGN",
      icon: Zap,
      description: t('programs.desc1'),
      features: [
        t('programs.feature1_1'),
        t('programs.feature1_2'),
        t('programs.feature1_3'),
        t('programs.feature1_4')
      ],
      cta: t('programs.cta1'),
      highlight: false
    },
    {
      tier: t('programs.tier2'),
      name: "Abundance Path™",
      price: "499-999",
      currency: "BGN",
      icon: Star,
      description: t('programs.desc2'),
      features: [
        t('programs.feature2_1'),
        t('programs.feature2_2'),
        t('programs.feature2_3'),
        t('programs.feature2_4'),
        t('programs.feature2_5'),
        t('programs.feature2_6')
      ],
      cta: t('programs.cta2'),
      highlight: true
    },
    {
      tier: t('programs.tier3'),
      name: t('programs.name3'),
      price: t('programs.price3'),
      currency: "",
      icon: Crown,
      description: t('programs.desc3'),
      features: [
        t('programs.feature3_1'),
        t('programs.feature3_2'),
        t('programs.feature3_3'),
        t('programs.feature3_4'),
        t('programs.feature3_5'),
        t('programs.feature3_6')
      ],
      cta: t('programs.cta3'),
      highlight: false
    }
  ];

  return (
    <section id="programs" className="py-16 md:py-20 bg-gradient-dark relative">
      <div className="container mx-auto px-4 md:px-6">
        <div className="max-w-6xl mx-auto">
          {/* Section Header */}
          <div className="text-center mb-12 md:mb-16">
            <h2 className="font-display text-3xl sm:text-4xl md:text-5xl font-bold mb-4 md:mb-6 px-4"
              dangerouslySetInnerHTML={{ __html: t('programs.heading') }}
            />
            <p className="text-base sm:text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto px-4">
              {t('programs.subheading')}
            </p>
          </div>

          {/* Programs Grid */}
          <div className="grid md:grid-cols-3 gap-6 md:gap-8">
            {programs.map((program, index) => {
              const Icon = program.icon;
              return (
                <div
                  key={index}
                  className={`relative p-6 md:p-8 rounded-xl md:rounded-2xl border transition-elegant ${
                    program.highlight
                      ? 'bg-gradient-primary border-primary shadow-elegant md:scale-105'
                      : 'bg-card border-border hover:border-primary/50 hover:shadow-gold'
                  }`}
                >
                  {/* Highlight Badge */}
                  {program.highlight && (
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                      <div className="px-4 py-1 bg-primary text-primary-foreground text-sm font-semibold rounded-full">
                        {t('programs.mostPopular')}
                      </div>
                    </div>
                  )}

                  {/* Icon */}
                  <div className={`w-12 h-12 md:w-16 md:h-16 rounded-lg md:rounded-xl flex items-center justify-center mb-4 md:mb-6 ${
                    program.highlight ? 'bg-background/20' : 'bg-primary/10'
                  }`}>
                    <Icon className={program.highlight ? 'text-background' : 'text-primary'} size={24} />
                  </div>

                  {/* Tier & Name */}
                  <div className="mb-4 md:mb-6">
                    <div className={`text-xs md:text-sm font-semibold mb-1 md:mb-2 ${
                      program.highlight ? 'text-background/70' : 'text-primary'
                    }`}>
                      {program.tier.toUpperCase()}
                    </div>
                    <h3 className={`font-display text-xl md:text-2xl font-bold mb-2 ${
                      program.highlight ? 'text-background' : 'text-foreground'
                    }`}>
                      {program.name}
                    </h3>
                    <p className={`text-xs md:text-sm ${
                      program.highlight ? 'text-background/80' : 'text-muted-foreground'
                    }`}>
                      {program.description}
                    </p>
                  </div>

                  {/* Price */}
                  <div className="mb-4 md:mb-6 pb-4 md:pb-6 border-b border-current/10">
                    <div className="flex items-baseline gap-2">
                      <span className={`text-3xl md:text-4xl font-display font-bold ${
                        program.highlight ? 'text-background' : 'text-foreground'
                      }`}>
                        {program.price}
                      </span>
                      {program.currency && (
                        <span className={`text-base md:text-lg ${
                          program.highlight ? 'text-background/70' : 'text-muted-foreground'
                        }`}>
                          {program.currency}
                        </span>
                      )}
                    </div>
                  </div>

                  {/* Features */}
                  <ul className="space-y-2 md:space-y-3 mb-6 md:mb-8">
                    {program.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start gap-2 md:gap-3">
                        <Check
                          className={`flex-shrink-0 mt-0.5 ${
                            program.highlight ? 'text-background' : 'text-primary'
                          }`}
                          size={18}
                        />
                        <span className={`text-xs md:text-sm ${
                          program.highlight ? 'text-background/90' : 'text-muted-foreground'
                        }`}>
                          {feature}
                        </span>
                      </li>
                    ))}
                  </ul>

                  {/* CTA */}
                  <Button
                    className={`w-full min-h-[48px] ${
                      program.highlight
                        ? 'bg-background text-primary hover:bg-background/90'
                        : ''
                    }`}
                    variant={program.highlight ? 'default' : 'premium'}
                    size="lg"
                  >
                    {program.cta}
                  </Button>
                </div>
              );
            })}
          </div>

          {/* Additional Info */}
          <div className="mt-12 text-center">
            <p className="text-muted-foreground mb-4">
              {t('programs.guarantee')}
            </p>
            <p className="text-sm text-muted-foreground">
              {t('programs.questions')} <button className="text-primary hover:underline">{t('programs.scheduleCall')}</button>
            </p>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Programs;
